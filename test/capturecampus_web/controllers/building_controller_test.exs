defmodule CapturecampusWeb.BuildingControllerTest do
  use CapturecampusWeb.ConnCase

  alias Capturecampus.Campus

  @create_attrs %{abbrev: "some abbrev", address: "some address", image_path: "some image_path", lat: 120.5, long: 120.5, name: "some name"}
  ...
  def fixture(:building) do
    {:ok, building} = Campus.create_building(@create_attrs)
    building
  end
  ...
  describe "update building" do
    ...
    test "redirects when data is valid", %{conn: conn, building: building} do
      ...
      assert html_response(conn, 200) =~ "some updated abbrev"
    end
